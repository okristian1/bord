...
def my_form_post():
    ...
    if request.method == 'POST':
        if request.form['submit'] == 'Søk':
            ...
            booking_start = datetime.strptime(get_time, '%Y-%m-%d %H:%M')
            date = booking_start.date()
            guests = request.form['guests']
            free_tables_banksalen = read_from_db(booking_start, date, "SpareBank 1%")
            # free_tables_aisuma = read_from_db(booking_start, date, "AiSuma%")
            # free_tables_frati = read_from_db(booking_start, date, "Frati%")
            # free_tables_eld = read_from_db(booking_start, date, "Restaurant Eld%")
            # free_tables_sostrenekarlsen = read_from_db(booking_start, date, "Søstrene Karlsen%")
            # free_tables_una = read_from_db(booking_start, date, "Una Pizzeria e Bar%")
...
def read_from_db(datetime, date, restaurant):
